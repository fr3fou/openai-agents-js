...
export interface NorthflankSandboxClientOptions extends SandboxClientOptions {
   ...
   * instead of provisioning a new one. When set, the volume is attached
   * to the service but is NOT deleted on `delete()` — the caller owns it.
   */
  volumeId?: string;
  ...
  /** Poll interval while waiting for the deployment to be ready. Defaults to 5s. */
...
export class NorthflankSandboxSession extends RemoteSandboxSessionBase<NorthflankSandboxSessionState> {
  ...
  /** Last-resort cleanup. Pauses if `pauseOnExit`, deletes otherwise. */
...
> {
  ...
  private async attachWorkspaceVolume(
    ...
    if (options.volumeId) {
      state.volumeId = options.volumeId;
